...
//        let chosenIndex: Int = self.index(of: card) // index 是个 函数   card 是 func choose(card: Card) 中选中的 card （已经被优化）
//        let chosenIndex: Int = cards.firstIndex(matching: card) //这就实现了 代码的复用
...
//    这个查找数组下标的 function 被 Grid.swift 中 extension Array where Element: Identifiable 代替了
